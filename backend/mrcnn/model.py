"""
Mask R-CNN implementation following the inference pipeline:
Input -> Resize/Normalize -> Backbone (ResNet) -> FPN -> RPN -> ROI Align -> Heads (Class, Box, Mask) -> Post-processing -> Output
"""
...
def compute_backbone_shapes(config, image_shape):
    ...
    assert config.BACKBONE in ["resnet101"]
    return np.array(
...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    block_count = {"resnet101": 22}[architecture]
...
############################################################
# Proposal Layer (RPN output refinement)
############################################################
...
class ProposalLayer(KE.Layer):
    """Takes RPN outputs and generates proposal boxes."""
...
class PyramidROIAlign(KE.Layer):
    """Crops and resizes ROIs from the FPN feature maps to a fixed size."""
    ...
    def call(self, inputs):
        ...
        for i, level in enumerate(range(2, 6)):
            ...
            pooled.append(tf.image.crop_and_resize(feature_maps[i], level_boxes, box_indices,
...
############################################################
# Detection Target Layer (only used in training)
############################################################
...
############################################################
# Detection Layer (post-processing)
############################################################
...
############################################################
# Feature Pyramid Network Heads (Class, Box, Mask)
############################################################
...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    anchors = utils.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES, config.RPN_ANCHOR_RATIOS,
...
############################################################
# MaskRCNN Class – main model container
############################################################
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # Step 5: Proposal Layer (refine anchors → proposals)
        ...
        if mode == "training":
            ...
            # Step 6: ROI Align + detection targets (for training)
            rois, target_class_ids, target_bbox, target_mask = \
                DetectionTargetLayer(config, name="proposal_targets")([target_rois, input_gt_class_ids, gt_boxes, input_gt_masks])
            # Step 7: Heads (Class, Box, Mask)
            mrcnn_class_logits, mrcnn_class, mrcnn_bbox = \
                fpn_classifier_graph(rois, mrcnn_feature_maps, input_image_meta, config.POOL_SIZE,
            ...
            # Step 7: Heads (Class, Box, Mask) on proposals
            mrcnn_class_logits, mrcnn_class, mrcnn_bbox = \
                fpn_classifier_graph(rpn_rois, mrcnn_feature_maps, input_image_meta, config.POOL_SIZE,
                                     config.NUM_CLASSES, train_bn=config.TRAIN_BN,
                                     fc_layers_size=config.FPN_CLASSIF_FC_LAYERS_SIZE)
            # Step 8: Detection Layer (post‑processing)
    ...
    def find_last(self):
        ...
        if not dir_names:
            raise FileNotFoundError("Could not find model directory under {}".format(self.model_dir))
        dir_name = os.path.join(self.model_dir, dir_names[-1])
        checkpoints = next(os.walk(dir_name))[2]
        checkpoints = filter(lambda f: f.startswith("mask_rcnn"), checkpoints)
        checkpoints = sorted(checkpoints)
        if not checkpoints:
            raise FileNotFoundError("Could not find weight files in {}".format(dir_name))
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            molded_image, window, scale, padding, crop = utils.resize_image(
                image, min_dim=self.config.IMAGE_MIN_DIM, min_scale=self.config.IMAGE_MIN_SCALE,
                max_dim=self.config.IMAGE_MAX_DIM, mode=self.config.IMAGE_RESIZE_MODE)
    ...
    def detect(self, images, verbose=0):
        ...
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_masks = \
                self.unmold_detections(detections[i], mrcnn_mask[i], image.shape, molded_images[i].shape, windows[i])
...
def compose_image_meta(image_id, original_image_shape, image_shape, window, scale, active_class_ids):
    meta = np.array([image_id] + list(original_image_shape) + list(image_shape) + list(window) +
                    [scale] + list(active_class_ids))
    return meta
...
def parse_image_meta(meta):
    ...
    return {"image_id": image_id.astype(np.int32), "original_image_shape": original_image_shape.astype(np.int32),
            "image_shape": image_shape.astype(np.int32), "window": window.astype(np.int32),
            "scale": scale.astype(np.float32), "active_class_ids": active_class_ids.astype(np.int32)}
...
def parse_image_meta_graph(meta):
    ...
    scale = meta[:, 11]
    active_class_ids = meta[:, 12:]
    return {"image_id": image_id, "original_image_shape": original_image_shape, "image_shape": image_shape,
